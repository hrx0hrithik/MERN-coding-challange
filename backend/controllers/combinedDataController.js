...
const getCombinedData = async (req, res) => {
    try {
      ...
      const statisticsResponse = await fetch(`${backendUrl}/api/statistics?month=${month}`);
      const barChartDataResponse = await fetch(`${backendUrl}/api/bar-chart?month=${month}`);
      const pieChartDataResponse = await fetch(`${backendUrl}/api/pie-chart?month=${month}`);
